...
func listSkillFilesTool(loaded map[string]agentskill.Entry) Tool {
	return Tool{
		...
		Handle: func(_ context.Context, call Call) (Result, error) {
			...
			base, relativeBase, err := safeSkillPath(entry, argumentText(call.Arguments, "path"))
			if err != nil {
				return Result{}, err
			}
			...
			err = filepath.WalkDir(base, func(path string, item os.DirEntry, walkErr error) error {
				...
				_, relative, pathErr := safeSkillPath(entry, relativeSkillPath(entry.InstallPath, path))
				if pathErr != nil {
					return pathErr
				}
...
func relativeSkillPath(root string, path string) string {
