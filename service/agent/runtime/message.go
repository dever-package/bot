...
func (s Service) RecordMessage(ctx context.Context, request MessageRequest) (map[string]any, error) {
	...
	if requestID != "" && role == "assistant" {
		if existing := messageModel.Find(ctx, map[string]any{
			...
			return map[string]any{"session": sessionMap(*session), "message": messageMap(existing)}, nil
		}
	}
	...
	return map[string]any{"session": sessionMap(*session), "message": messageMap(message)}, nil
}

func (s Service) BeginRunTurn(ctx context.Context, request RunTurnRequest) error {
	owner, err := currentOwner(ctx)
	if err != nil {
		return err
	}
	...
	return orm.Transaction(ctx, func(tx context.Context) error {
		...
		now := time.Now()
		if uint64(messageModel.Insert(tx, map[string]any{
			"session_id": session.ID,
			"role":       "user",
			"kind":       "chat",
			"text":       input,
			"content":    encodeJSON(map[string]any{"format": "markdown", "text": input}, "{}"),
			"output":     "{}",
			"request_id": "",
			"status":     agentmodel.MessageStatusNormal,
			"created_at": now,
