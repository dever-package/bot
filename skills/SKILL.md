...
- 分类是知识文件目录和向量集合的命名空间，知识库创建后不允许直接改分类；同一分类启用向量时必须使用相同向量能力。
- 原文工具和索引工具必须执行同一套文档状态、过期和审核规则；开启严格审核后，内容变化必须回到待审核。
- 重索引必须使用数据库租约防止多实例并发，并在新节点事务提交前保留上一次成功索引；runtime 读取文件时不得触发全库文件同步。
