...
- 结果内容超出容器时支持贴边滚动，滚动条仅在内容区域悬停时显示，不再裁掉正文。
...
滚动条默认透明，鼠标进入可滚动内容区域时才显示滑块；细滚动槽宽度保持稳定，避免显隐引起正文横向跳动。没有内容溢出时，`overflow-y: auto` 不绘制滚动条。Markdown 标题、列表、引用、代码、表格，以及 rich JSON 中的图片、视频、音频和文件，全部沿用共享 `ContentView`。纯图片或视频填充结果卡时继续保持 `object-fit`，不会降级成 JSON。
