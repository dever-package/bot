...
type KnowledgeBase struct {
	...
	ReviewRequired      bool       `dorm:"type:boolean;not null;default:false;comment:是否要求审核通过"`
	...
	IndexWorkerID       string     `dorm:"type:varchar(128);not null;default:'';comment:索引执行者"`
	IndexLeaseExpiresAt *time.Time `dorm:"type:timestamp;null;comment:索引租约过期时间"`
	IndexHeartbeatAt    *time.Time `dorm:"type:timestamp;null;comment:索引心跳时间"`
...
type KnowledgeBaseIndex struct {
	...
	IndexLease     struct{} `index:"index_lease_expires_at,id"`
